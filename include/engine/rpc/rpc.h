...
private:
    ...
    static void           init_s(write_t w, int64_t guidApp);

public:
    //-----------------------------------------------------------------------------
    void init(int64_t guid);
    int  size();
    int  close();
...
public:
    ...
    {
        static_assert(function_traits<t_fun>::callable == 1, "need fun!");
    ...
    {
        static_assert(function_traits<t_fun>::callable == 1, "need fun!");
        return m_server.register_<int32_t(head_code_t::RPC_ROUTE)>(std::move(fun));
    }
    //-----------------------------------------------------------------------------
    template <int32_t type_ = int32_t(head_code_t::REQUEST), typename t_req, typename... Args>
    int32_t request(write_t post, t_req&& req, Args&&... args)
    {
        using args_tuple_t    = std::tuple<Args...>;
        constexpr int arg_n_t = std::tuple_size_v<args_tuple_t>;
        static_assert(arg_n_t <= 1, "too mutch arg!");
        //-----------------------------------------------------------------------------
        nullptr == post ? post = s_post : nullptr;
        //-----------------------------------------------------------------------------
        try {
            if constexpr (arg_n_t == 1) {
                ...
                if constexpr (function_traits<arg_t>::callable == 1) {
                    ...
                    if constexpr (cb_traits_t::arg_size == 1) {
                        ...
                        return m_client.request<false, type_>(post, std::move(cb), std::forward<t_req>(req), rep);

                    } else {
                        //异步调用，无返回值
                        return m_client.request<false, type_>(post, std::move(cb), std::forward<t_req>(req));
                    }
                    ...
                    return m_client.request<true, type_>(post, std::move(cb), std::forward<t_req>(req), rep);
                }

            } else if constexpr (arg_n_t == 0) {
                //同步调用，无返回值
                auto cb = []() {
                };
                return m_client.request<true, type_>(post, std::move(cb), std::forward<t_req>(req));
            }
    ...
    int32_t call(t_fun fun, t_req&& req, Args&&... args)
    {
        ...
        constexpr int arg_n_t = std::tuple_size_v<args_tuple_t>;
        //-----------------------------------------------------------------------------
        static_assert(fun_traits_t::callable == 1, "fun need callable!");
        ...
        if constexpr (fun_traits_t::arg_size == 1) {
            ...
            using fn_rep_t    = typename fun_traits_t::args<1>::type;
            using arg_t       = std::tuple_element_t<0, args_tuple_t>;
            using cb_traits_t = function_traits<std::remove_reference_t<arg_t>>;
            ...
            if constexpr (cb_traits_t::callable == 1) {
                ...
                using cb_rep_t = typename cb_traits_t::args<0>::type;
        ...
        return this->request<int32_t(head_code_t::RPC_ROUTE)>(s_post, std::forward<t_req>(req), std::forward<Args>(args)...);
    }
    //-----------------------------------------------------------------------------
    template <typename Arg>
    int32_t package(int64_t guidApp, int64_t guid, channel_t::data_t req, Arg&& arg)
    {
