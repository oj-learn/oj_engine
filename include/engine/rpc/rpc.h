...
public:
    ...
    int post(t_req&& req)
    {
        return m_client.request<-1, int(head_code_t::RPC_ROUTE)>(s_post, ([]() {}), std::forward<t_req>(req));
    }
    //-----------------------------------------------------------------------------
    ...
    int package(long guidApp, long guid, channel_t::data_t req, Arg&& arg)
    {
        ...
        if constexpr (cb_traits_t::callable == 1) {
            ...
            return m_client.callAsync(s_post, false, guidApp, guid, req, std::move(arg));

        } else {
            static_assert(std::is_same_v<channel_t::data_t, std::remove_reference_t<Arg>>, "arg need data_t!!");

            return m_client.callAsync(s_post, true, guidApp, guid, req, [&arg](channel_t::data_t data) {
                arg = data;
            });
        }
    }
            ...
            if constexpr (arg_n_t == 1) {
                ...
                if constexpr (function_traits<arg_t>::callable == 1) {
                    ...
                    if constexpr (cb_traits_t::arg_size == 1) {
                        ...
                        return m_client.request<0, type_>(w, std::move(cb), std::forward<t_req>(req), rep);

                    } else {
                        //异步调用，无返回值
                        return m_client.request<0, type_>(w, std::move(cb), std::forward<t_req>(req));
                    }
                    ...
                    return m_client.request<1, type_>(w, std::move(cb), std::forward<t_req>(req), rep);
                }
            }

            if constexpr (arg_n_t == 0) {
                //同步调用，无返回值
                return m_client.request<1, type_>(w, ([]() {}), std::forward<t_req>(req));
            }
